...
west build -d build/left -p -b nice_nano_v2 -- \
  -DSHIELD="little_wing_left nice_view" \
...
west build -d build/right -p -b nice_nano_v2 -- \
  -DSHIELD="little_wing_right nice_view" \
  -DZMK_CONFIG="/workspaces/zmk-config/config" \
  -DZMK_EXTRA_MODULES="/workspaces/zmk-config;/workspaces/zmk-config/drivers/pmw3610" \
  -DKEYMAP_FILE="/workspaces/zmk-config/config/little_wing.keymap;/workspaces/zmk-config/config/little_wing_right.keymap"
